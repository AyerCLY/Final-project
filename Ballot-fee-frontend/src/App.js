...
function App() {
  ...
  const contractAddress = '0xDF87B51e0b43c88B020f147282A9EeD934b41502';
...
//Approve
  const setApproveHandler = async (event) => {
    try {
      event.preventDefault();
      if (window.ethereum) {
...
//Transfer
  const setTransferHandler = async (event) => {
    try {
      event.preventDefault();
      if (window.ethereum) {
        ...
        let txn = await BallotContract.withdrawMoney();
        await txn.wait(); 
      ...
      <section className="customer-section px-10 pt-5 pb-10">
        ...
        <div className="mt-7 mb-9">
          <form className="form-style">
            <button
              className="btn-red"
              onClick={setApproveHandler}> Approve Button</button>
      ...
      </section>
      {
        isBallotOwner && (
          <section className="ballot-owner-section">
            <h2 className="text-xl border-b-2 border-indigo-500 px-10 py-4 font-bold">Ballot Fee Admin Panel</h2>
            <div className="p-10">
              <form className="form-style">
                <input
                  type="text"
                ...
                <button
                  className="btn-grey"
                  onClick={setTransferHandler}>Transfer Button</button>
              </form>
            </div>
          </section>
        )
